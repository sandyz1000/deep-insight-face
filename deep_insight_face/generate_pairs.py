...
def transform_to_lfw_format(image_directory: str,
                            num_processes: Optional[int] = cpu_count(logical=False),
                            debug: bool = False):
    ...
    process_pool = ThreadPool(1 if debug else num_processes)
...
def write_pairs_to_file(fname: str,
    ...
    with io.open(fname, 'w', io.DEFAULT_BUFFER_SIZE, encoding='utf-8') as fpairs:
...
def _total_match(total_matches, n_class):
    tolerance = 1.2
    max_total_matches = max(total_matches, n_class)
    if max_total_matches > n_class and float(max_total_matches / n_class) > tolerance:
        return int(n_class * tolerance)

    return max_total_matches
...
def _make_matches(image_dir: str,
    ...
    curr_matches = 0
    total_matches = _total_match(total_matches, len(people))
...
def generate_pairs(
    ...
    transform_to_lfw_format(image_dir, debug=False)
    ...
    for fold in people_folds:
        ...
        mismatches.append(_make_mismatches(image_dir,
                                           ...
                                           num_matches_mismatches))
    if write_to_file:
        pairs_file_name = os.path.join(image_dir, pairs_file_name)
